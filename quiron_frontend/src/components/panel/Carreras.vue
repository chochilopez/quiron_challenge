...
export default {
  setup () {
    ...
    function functionAccionEditar (props) {
      storeCarrera.setAccionCarrera('editar')
      storeCarrera.setCarrera(props.row)
    ...
    function functionAccionVer (props) {
      storeCarrera.setAccionCarrera('ver')
      storeCarrera.setCarrera(props.row)
      vueRouter.push({ name: 'Carrera' })
    }
    ...
    async function asyncFunctionBorrarCarrera () {
      try {
        ...
        const result = await storeCarrera.delCarrera(letCarrera.id)
    ...
    async function asyncFunctionBuscarCarreraPorId () {
      try {
        letCargando.value = true
        const datos = await storeCarrera.getCarreraPorId(letCarrera.id)
        if (datos.status === 200) {
          letCargando.value = false
          storeCarrera.setAccionCarrera('ver')
          storeCarrera.setCarrera(await datos.data)
