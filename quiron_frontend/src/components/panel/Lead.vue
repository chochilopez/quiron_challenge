...
export default {
  setup () {
    ...
    if (storeEstudiante.lead !== null) {
      asyncFunctionBuscarLeadCarreraPorIdLead(storeEstudiante.lead.id)
      letEditar.value = false
      letVer.value = false
      letCrear.value = false
      Object.assign(letLead, storeEstudiante.lead)
    ...
    } else if (storeEstudiante.accionEstudiante === 'crear') {
      letCrear.value = true
    } else {
    ...
    async function asyncFunctionGuardarLead () {
      if (letFormulario.value.validate()) {
        try {
          ...
          letCargando.value = false
          const mensaje = 'Hubo un error al guardar el lead: ' + err.message
          console.error(mensaje)
          servicioAlertas.alertaError(mensaje)
        }
      } else {
