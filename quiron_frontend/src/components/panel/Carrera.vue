...
export default {
  setup () {
    ...
    if (storeCarrera.carrera !== null) {
      letEditar.value = false
      letVer.value = false
      Object.assign(letCarrera, storeCarrera.carrera)
      if (storeCarrera.accionCarrera === 'ver') {
        letVer.value = true
      } else if (storeCarrera.accionCarrera === 'editar') {
    ...
    async function asyncFunctionAgregarMateriaACarrera (props) {
      try {
        ...
        const result = await storeCarrera.agregarMateriaACarrera(objeto)
        ...
        letCargando.value = false
        functionOcultar()
        const error = 'Hubo un error al intentar agregar la materia a la carrera ' + err
    ...
    async function asyncFunctionBorrarMateriaDeCarrera () {
      try {
        ...
        const result = await storeCarrera.delMateriaDeCarrera(objeto)
        if (result.status === 200) {
        ...
        letCargando.value = false
        functionOcultar()
        const error = 'Hubo un error al intentar borrar la materia a la carrera ' + err
        servicioAlertas.infoError(error)
        console.error(error)
      }
    }
    ...
    async function asyncFunctionGuardarCarrera () {
      if (letFormulario.value.validate()) {
        try {
          ...
          const result = await storeCarrera.saveCarrera(objeto)
