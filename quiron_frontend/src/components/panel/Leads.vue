...
export default {
  setup () {
    ...
    function functionAccionAgregarLead () {
      storeEstudiante.setLead(null)
      storeEstudiante.setAccionEstudiante('crear')
    ...
    function functionAccionEditar (props) {
      ...
      storeEstudiante.setLead(props.row)
      vueRouter.push({ name: 'Lead' })
    }
    ...
    async function asyncFunctionBorrarLead () {
      try {
        ...
        const result = await storeEstudiante.delEstudiante(letLead.id)
    ...
    async function asyncFunctionBuscarLeadPorIdYEstado () {
      try {
        ...
        if (datos.status === 200) {
          letCargando.value = false
          storeEstudiante.setLead(await datos.data)
          storeEstudiante.setAccionEstudiante('ver')
    ...
    async function asyncFunctionBuscarLeads () {
      try {
        letCargando.value = true
        const datos = await storeEstudiante.getLeads()
        if (datos.status === 200) {
          letCargando.value = false
